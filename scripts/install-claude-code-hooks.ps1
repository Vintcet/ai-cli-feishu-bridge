...
function New-BridgeHookCommand {
    ...
    # Claude Code runs hook commands through bash on Windows, and bash re-parses the
    # quoting, so the command must not be wrapped in cmd.exe /d /s /c "…": bash would
    # consume the outer quotes, cmd.exe would never receive a /c command, fall back to
    # interactive mode, and treat the hook JSON on stdin as a command line.
    $command =
