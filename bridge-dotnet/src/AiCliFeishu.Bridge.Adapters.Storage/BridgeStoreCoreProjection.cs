...
public sealed record BridgeStoreCoreState(
    SessionDirectoryState Sessions,
    MessageRouteRegistryState Routes,
    ApprovalRegistryState Approvals);

public static class BridgeStoreCoreProjection
{
    public static BridgeStoreCoreState Project(BridgeStoreSnapshot snapshot)
    {
    ...
    public static InputRegistryState ProjectInputs(BridgeStoreSnapshot snapshot)
    {
        ArgumentNullException.ThrowIfNull(snapshot);
        return BridgeStoreInputPersistence.Project(snapshot.Sessions);
    }
