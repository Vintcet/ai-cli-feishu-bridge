...
Temporary-error retry handles recognizable 400/408/409/429/5xx, high-demand, busy-service, and timeout failures that are safe to replay. Configure 1–999 retries per consecutive failure batch, a 1–600 second base delay, and 0–120 seconds of random jitter. Codex, Claude Code, and OpenCode ask the same session to retry its previous task. Retry error cards include **Stop automatic retry**; if an attempt has already started, the action stops every later attempt in that batch. Any successful completion immediately resets the batch, so a later failure starts again at attempt 1 even when it happens moments later. Automatic retry writes only to a live assistant-managed window or a connected OpenCode session. Externally launched Codex/Claude Code sessions receive the error notification but are never injected with a retry and never resumed in a second background process. Codex sampling failures may write `task_complete.error` without running the `Stop` hook, so the bridge polls each active transcript from its current end and processes only newly appended errors.

Automatic approval handling uses the same risk rules for Codex, Claude Code, and OpenCode. It has three tiers and defaults to manual.

- **All manual** — every request waits for a person in Feishu.
- **Low risk only** (recommended) — an allowlist: only explicitly recognized read-only commands (`git status`/`diff`/`log`/`show`, `rg`, `ls`/`dir`/`Get-Content`) and in-project reads are approved. Builds, tests, and writes stay manual.
- **All but high risk** — a denylist: approve by default, refuse only what is irreversible or reaches beyond the project. Builds, tests, dependency installs, local scripts, in-project edits, `git add`/`commit`/`stash`/branch switches/`rebase`/`--amend`/non-force `push`, process termination, and read-only network calls all go through.

Under the relaxed tier these still wait for a person: deletion (`rm`, `del`, `Remove-Item`, `shred`, `dd`), disk and filesystem operations (`mkfs`, `format`, `diskpart`), database destruction (`DROP`, `TRUNCATE`, `DELETE FROM`), irreversible Git (`reset --hard`, `clean`, `checkout --`, `restore`, `branch -D`, `filter-branch`, `reflog expire`, `push --force`), privilege and system changes (`sudo`, `runas`, `chmod`, `chown`, `icacls`, `takeown`, `systemctl`, `schtasks`, `shutdown`), cloud and infrastructure tools (`terraform`, `kubectl`, `helm`, `aws`, `az`, `gcloud`, `docker`, `ssh`, `scp`, `rsync`), irrevocable outward actions (publish, upload, send, pay), piping a downloaded script straight into an interpreter (`curl … | bash`), outbound requests carrying a body or an uploaded file, credential and sensitive-config access (`.ssh`, `.aws`, `.env`, `control-token`), paths outside the project, and commands that name their target through a variable (`$env:X`, `%X%`).

Both tiers share one floor: **anything that cannot be fully inspected stays manual.** Truncated, oversized, or malformed input, excessive nesting, unbalanced quotes, and tools outside the assessable allowlist (including OpenCode permission requests, which currently arrive without a tool name or argument preview) are never auto-approved. That rule is what makes an approve-by-default tier safe; without it one oversized command would bypass every content rule.

Manual requests appear in Feishu first; the PC dialog opens only after **Transfer to PC approval** is selected, or automatically when Feishu delivery is unavailable. Successful automatic approvals are silent by default; enable the separate audit-card setting to send one resolved information card without action buttons. Automatic processing never silently allows an uncertain operation.
