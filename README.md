...
权限就绪后，助手创建或恢复的每个 Codex、Claude Code 或 opencode 窗口都会绑定一个独立私有飞书群，群内只有绑定用户和机器人；群名按运行时使用 `Codex｜`、`Claude｜` 或 `OpenCode｜` 前缀，设置会话别名后会同步更新。会话群中的所有普通文字（包括“状态”“帮助”等词）都会原样交给对应助手；机器人私聊才是绑定、状态、会话列表、别名和帮助等管理入口。权限暂未开通时，本地 CLI 仍可正常运行，桌面端“飞书群”列会显示创建失败及具体原因；通知会暂时回退到私聊。开通并发布新应用版本后，在客户端选中会话并点击“创建飞书群”即可重试。
