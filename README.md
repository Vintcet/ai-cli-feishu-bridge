...
- “设置”中可选择飞书新建项目使用的默认工作区，并控制普通过程信息、电脑端输入同步、临时错误自动重试和审批自动允许；设置立即生效并保存在本机。

点击控制面板右上角关闭或最小化时，窗口会隐藏到系统托盘；双击托盘图标可恢复，右键选择“退出”才会关闭控制面板进程。再次双击 EXE 会直接唤回已运行的窗口。退出控制面板仍不会停止后台桥接服务。

“临时错误自动重试”只处理能够识别和安全重放的 400/408/409/429/5xx、高负载、服务繁忙及超时错误。可配置重试 1～20 次、基础间隔 1～600 秒，以及随机增加 0～120 秒；每次实际等待为“基础间隔 + 0～随机增加秒”。托管窗口会在原会话中请求 Codex 重试上一项任务。Codex 的服务端采样失败可能直接写入 `task_complete.error` 而不执行 `Stop` Hook，因此桥接器会从活动转录当前文件末尾开始轮询，只处理之后新追加的错误，避免重放旧通知。“审批自动允许”具有明显风险，默认关闭，首次开启时桌面助手会再次确认；开启后审批会直接处理，本机不再弹出审批窗口。
...
- opencode 请求权限时会发送飞书审批卡片：飞书“允许”对应 `once`，“拒绝”对应 `reject`；兼容新版 `/permission/{id}/reply` 和旧版会话权限端点，当前卡片暂不暴露 `always`；
