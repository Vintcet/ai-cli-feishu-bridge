...
- 429、503、高负载、服务繁忙和超时等临时错误使用独立错误卡通知；即使 Codex 未触发 `Stop` Hook，也会从活动 JSONL 转录中识别并推送，并可配置自动重试次数与随机间隔；
...
- “设置”中可选择飞书新建项目使用的默认工作区，并控制普通过程信息、电脑端输入同步、临时错误自动重试、审批自动允许及自动审批留痕；设置立即生效并保存在本机。

点击控制面板右上角关闭或最小化时，窗口会隐藏到系统托盘；双击托盘图标可恢复，右键选择“退出”才会关闭控制面板进程。再次双击 EXE 会直接唤回已运行的窗口。退出控制面板仍不会停止后台桥接服务。

“临时错误自动重试”只处理能够识别和安全重放的 400/408/409/429/5xx、高负载、服务繁忙及超时错误。可配置重试 1～20 次、基础间隔 1～600 秒，以及随机增加 0～120 秒；每次实际等待为“基础间隔 + 0～随机增加秒”。托管窗口会在原会话中请求 Codex 重试上一项任务。Codex 的服务端采样失败可能直接写入 `task_complete.error` 而不执行 `Stop` Hook，因此桥接器会从活动转录当前文件末尾开始轮询，只处理之后新追加的错误，避免重放旧通知。

“审批自动允许”对进入桥接器的 Codex、Claude Code 和 OpenCode 权限请求使用同一套语义，具有明显风险且默认关闭，首次开启时桌面助手会再次确认。开启后先直接处理审批，成功时默认不发送待审批卡、已处理卡或本机弹窗；只有同时开启“自动审批后发送处理留痕”时，才会发送一张没有操作按钮的已处理信息卡。若自动处理失败，请求会保留为待审批，并回退到可操作的飞书卡和本机审批窗口，不会静默挂起。
...
项目根目录中的 `启动Codex助手.vbs` 和 `停止Codex助手.vbs` 只控制后台桥接服务，可作为备用手动方式，但不会打开桌面面板。
...
附件用法：在独立会话群里先发送图片或文件，下一条直接发 `分析这个附件`；在机器人私聊且有多个会话时，下一条写成 `@别名 分析这个附件`。最短的文件回传测试是在会话群发送 `发文件 生成一个 test.txt 并发回来`，或在私聊发送 `发文件 @别名 生成一个 test.txt 并发回来`。附件会保存到 `data/uploads/<月份>/`，单个默认最多 25 MiB，并定期清理 7 天前的暂存文件。
