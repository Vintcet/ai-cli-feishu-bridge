...
“低风险审批自动允许”对进入桥接器的 Codex、Claude Code 和 OpenCode 权限请求使用同一套风险规则，默认关闭。开启后，未命中删除/破坏性 Git、依赖安装发布、网络与云端操作、权限或系统配置修改、敏感路径及项目目录外路径等规则的请求会直接允许；命中高风险规则的请求仍发送可操作的飞书卡并保留本机审批入口。自动允许成功时默认不发送卡片；同时开启“自动审批后发送处理留痕”时，才发送一张没有操作按钮的已处理信息卡。自动处理失败或审批超时都会回退到人工/本机确认，不会静默放行。
