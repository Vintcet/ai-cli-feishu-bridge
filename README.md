...
- 三种运行时在本轮完成、出错或等待补充信息时发送飞书通知；
- Codex、Claude Code 和 OpenCode 的权限请求优先发送飞书审批卡片，需要时可转回 PC 审批；
- 在飞书批准或拒绝，也可在桌面助手本机审批窗口处理，两端状态自动同步；
- 托管会话运行中可通过飞书直接插话，或明确排队到下一轮；
- Codex `request_user_input`、Claude Code `AskUserQuestion` 和 OpenCode 问题均可通过飞书回答；
...
- 400/408/409/429/5xx、高负载、服务繁忙和超时等临时错误使用独立错误卡通知，并对 Codex、Claude Code 和 OpenCode 使用统一的连续失败重试计数；即使 Codex 未触发 `Stop` Hook，也会从活动 JSONL 转录中识别并推送；
...
- “历史记录”包含助手启动过或由 Hook 识别、当前已不在线的 Codex / Claude Code 会话，以及通过本机端口接入过的 OpenCode 当前对话；外部会话从历史继续后会转为助手托管；
...
- “设置”中可选择飞书新建项目使用的默认工作区，并控制普通过程信息、电脑端输入同步、临时错误自动重试、权限请求自动处理档位及自动审批留痕；设置立即生效并保存在本机。
...
“临时错误自动重试”只处理能够识别和安全重放的 400/408/409/429/5xx、高负载、服务繁忙及超时错误。可配置每批连续失败重试 1～999 次、基础间隔 1～600 秒，以及随机增加 0～120 秒；每次实际等待为“基础间隔 + 0～随机增加秒”。Codex、Claude Code 和 OpenCode 都会在原会话中请求重试上一项任务；自动重试错误卡底部会显示“停止自动重试”，重试已经发出时点击它会停止本批后续尝试。任意一次成功完成都会立即清零本批计数，之后即使很快再次失败也会从第 1 次重新计算。自动重试只会写入仍在线的助手托管窗口或已连接的 OpenCode 会话；外部手动打开的 Codex / Claude Code 只接收错误通知，不会被桥接器注入重试，也不会另起后台会话。Codex 的服务端采样失败可能直接写入 `task_complete.error` 而不执行 `Stop` Hook，因此桥接器会从活动转录当前文件末尾开始轮询，只处理之后新追加的错误，避免重放旧通知。

“权限请求自动处理”对进入桥接器的 Codex、Claude Code 和 OpenCode 权限请求使用同一套风险规则，分三档，默认“全部人工确认”。

- **全部人工确认**：每个请求都发送飞书卡等待处理。
- **仅自动允许低风险**（推荐）：白名单式，只放行明确的只读命令（`git status`/`diff`/`log`/`show`、`rg`、`ls`/`dir`/`Get-Content` 等）与项目内读取；构建、测试、写文件在内的其余请求一律人工确认。
- **除高危外自动允许**：黑名单式，默认放行，只拦不可逆或影响超出项目的操作。构建、测试、安装依赖、执行本机脚本、改动项目内文件、`git add`/`commit`/`stash`/切分支/`rebase`/`--amend`/非 force 的 `push`、终止进程、只读网络请求都会直接放行。

“除高危外自动允许”下仍然必须人工确认的是：删除类命令（`rm`/`del`/`Remove-Item`/`shred`/`dd`）、磁盘与文件系统操作（`mkfs`/`format`/`diskpart`）、数据库破坏（`DROP`/`TRUNCATE`/`DELETE FROM`）、不可逆 Git（`reset --hard`、`clean`、`checkout --`、`restore`、`branch -D`、`filter-branch`、`reflog expire`、`push --force`）、提权与系统配置（`sudo`/`runas`/`chmod`/`chown`/`icacls`/`takeown`/`systemctl`/`schtasks`/`shutdown`）、云与基础设施（`terraform`/`kubectl`/`helm`/`aws`/`az`/`gcloud`/`docker`/`ssh`/`scp`/`rsync`）、对外不可撤销动作（发布、上传、发消息、支付）、下载后直接管道执行远程脚本（`curl … | bash`）、携带请求体或上传文件的外发请求、访问凭据与敏感配置（`.ssh`/`.aws`/`.env`/`control-token` 等）、越出项目目录的路径，以及通过变量引用目标（`$env:X`/`%X%`）的命令。

此外，两档都遵守同一条底线：**参数无法完整解析时一律转人工**——被截断、超长、非法 JSON、嵌套过深、引号不闭合，以及不在可评估白名单内的工具（含 OpenCode 当前不上报工具名与参数的权限请求），都不会自动放行。这是“默认放行”这一档的安全前提，否则一条超长命令就能绕过全部内容规则。

人工审批默认只发送飞书卡，不会同时弹出电脑端窗口；点击卡片中的“转回 PC 审批”或引用回复“本机确认”后，电脑端才会弹出，飞书不可用时则自动回退到 PC。自动允许成功时默认不发送卡片；同时开启“自动审批后发送处理留痕”时，才发送一张没有操作按钮的已处理信息卡。自动处理失败或审批超时不会静默放行。
...
关闭窗口后，会话会从“活跃会话”移动到“历史记录”。继续历史会话时，助手沿用保存的工作目录和管理员模式，并安全传入 `resume 会话ID`；不需要复制或输入完整 ID。外部终端通过参数恢复的 Codex / Claude Code 在关闭后同样进入历史，但外部窗口在线时仍不接受普通飞书输入；从历史点击“继续对话”后，新窗口会转为助手托管并获得飞书双向控制能力。若直接在已有飞书会话群继续发送消息，托盘中的桌面面板也会执行同一恢复流程。仍在线的会话不会同时出现在历史记录中。若曾点击“删除记录”，该结束会话保持隐藏；一旦它被再次恢复，隐藏标记会自动清除，之后关闭时会重新出现在历史记录中。桥接器启动时也会修复旧版本遗留的“活动但仍被隐藏”记录。
...
新建命令只允许已绑定管理员使用，可从机器人私聊或会话群里的飞书斜杠命令 `/新建` 打开；英文 `/new` 继续兼容。项目名是单层文件夹名称，可以包含中文和空格，但不能包含盘符、斜杠、`..` 或 Windows 文件名保留字符。桌面面板必须仍在运行或托盘中；飞书启动的新窗口默认使用普通权限，不会远程触发管理员提升。默认工作区可在桌面“设置”中修改；首次升级时默认使用桥接器目录的上一级。
...
运行状态和审计数据存放在 `data/`，飞书凭据单独保存在项目根目录的 `.env`；项目不提供或使用自己的中转服务器：

- `bindings.json`、`sessions.json`、`message-routes.json`、`approvals.json`、`settings.json`：绑定、会话、路由、审批和设置；
- `control-token.json`：首次启动时生成的随机本机控制令牌；
- `approval-events.log` 及轮换备份：审批请求、通知和决定来源的本机审计记录；
