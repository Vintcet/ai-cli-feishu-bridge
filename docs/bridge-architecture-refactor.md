...
- 桌面控制层已新增尚未接入 UI/启动流的生产切换组合根：它把生产回环端点、严格的 64 位十六进制控制令牌、正式只读 Store 交接检查器、持久化协调器、恢复观察器、恢复执行器和 Node/C# 真实进程工厂绑定在同一固定端口与 `production-dotnet` 实例上；构造阶段只读配置，不探测网络、不启动或停止进程、不写检查点、不取得租约，也不接受环境变量直接选择 C# Production。切换事务仍没有被 `BridgeClient`、`MainForm`、`Program` 或桌面启动配置实际调用；Host 仅具备由耐久检查点显式授权的 Active 启动入口，不存在启动失败时静默切换。当前生产检查点尚未建立，Node 仍是唯一 Active Owner。
