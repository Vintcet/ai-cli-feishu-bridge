...
- 切换事务、持久化协调器、恢复执行器和真实进程适配器仍没有被 `BridgeClient`、`MainForm`、`Program` 或桌面启动配置接入；Host 仅具备由耐久检查点显式授权的 Active 启动入口，不存在环境变量或启动失败时静默切换。当前生产检查点尚未建立，Node 仍是唯一 Active Owner。
