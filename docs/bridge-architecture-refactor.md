...
- Active OpenCode Transport Owner 自有禁用重定向、代理和 Cookie 的 HTTP 传输，只向 Endpoint Directory 生成的固定 IPv4 回环 Origin 发送命令；提示使用 15 秒超时，权限、问题和中止使用 10 秒超时，不做自动重试，也不读取或回显错误响应体。权限回复只在 404/405 时按 v2、modern、legacy 顺序兼容回退；每条请求及每次回退前后都复核 `(port,generation,session)` 目标租约，端点替换、ready 撤销或 session 转移会立即拒绝后续发送和完成。启动、恢复、等待就绪与停止仍只委托 OpenCode RuntimeLifecycle 端口；该生产 Owner 尚未实现，因此 Active 静态预检继续 fail closed，Node 仍是唯一生产 Owner；
